...
duracoes = list(range(passo, duracao_max + passo, passo))
...
for duracao in duracoes:
    ...
    disparidades.append(disparidade)

    print(
        f"Janela {inicio}s a {fim}s "
        f"({duracao}s): disparidade = {disparidade:.4f}"
    )
...
plt.xlabel("Duração da janela (s)")
plt.ylabel("Disparidade Procrustes")

plt.title(
    f"Disparidade Procrustes sem spline\n"
    f"Início fixo em {inicio}s, final até {inicio + duracao_max}s, passo de {passo}s"
)
